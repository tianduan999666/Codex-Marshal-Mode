...
**待闭环验证**：
- ⏸️ 脚手架-门禁章节名一致性（已修正，待验证）
- ⏸️ 状态机门禁守住本仓 PowerShell 代码（已修正，待验证）
- ⏸️ 触发条件完整性检查（estimated_hours > 4 自动触发，待验证）
- ⏸️ 完整干跑验证（创建测试任务包 → 门禁检查 → 通过/拦截）
...
**状态机门禁**：
- `drafting` 状态：允许编辑任务包文件，禁止修改任务包外代码文件
- `planning` 状态：允许编辑 tech-spec.md，禁止修改任务包外代码文件
- `ready` / `running` 状态：允许修改代码文件

**预期效果**（待实测验证）：
- 方向性错误减少（目标：显著降低，待建立基线）
- 返工率降低（目标：显著降低，待建立基线）
- 代码质量提升（目标：显著提升，待建立基线）
- 为 Phase 2 多 Agent 编排铺平道路（Planner Agent 独占 tech-spec.md 写入权）

**测量方式**（待建立）：
- 基线数据：记录当前 3-5 个任务的返工次数、方向修正次数
- 对比数据：使用 tech-spec.md 后的 3-5 个任务
- 观察周期：1-2 周
- 量化指标：返工次数、git revert 次数、decision-log.md 中的"路线修正"条目数
...
### 待验证项
- [ ] 完整干跑验证（创建测试任务包 → 填写 tech-spec.md → 门禁检查 → 通过）
- [ ] 状态机门禁实测（drafting 状态修改 .ps1 文件 → 拦截）
- [ ] 建立基线数据（记录当前 3-5 个任务的返工次数）
- [ ] 对比数据收集（使用 tech-spec.md 后的 3-5 个任务）
- [ ] 量化效果验证（1-2 周观察周期后评估）

### 已知问题与修正
1. ✅ 脚手架-门禁章节名不一致（已修正：统一为无编号章节名）
2. ✅ 状态机门禁未守住 PowerShell 代码（已修正：增加 .ps1/.psm1 后缀，排除任务包内文件）
3. ⏸️ 真源口径分裂（待修正：清稿总结文档状态调整）
4. ⏸️ 触发条件完整性（待增强：门禁检查 estimated_hours 字段）
