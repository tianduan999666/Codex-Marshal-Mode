...
- 当前对外主链已经不冗余：只剩 `传令：XXXX` 这 1 个做事入口，加 `传令：状态 / 传令：版本 / 传令：升级` 这 3 个可查命令。
- 当前还没完全超越 `ECC` 的地方，是还没有“你随便说一句人话，面板自动改写成正式入口”的零前缀代理层；第一次冷启动也仍建议先走 PowerShell 初始化。
- 当前已经强过 `ECC` 的地方，是中文口径更统一，固定开场白与 6 行状态栏更可感知，边界提示已经门禁化，同版本也已支持关键文件哈希护栏后跳过重复验真。
- 如果要看完整对照和下一刀建议，直接看：`docs/30-方案/09-V4-本机生产切换最小闭环方案.md`。
