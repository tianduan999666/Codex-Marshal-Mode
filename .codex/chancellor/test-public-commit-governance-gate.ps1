...
$checklistStepSummaryLineText = '5. 如需再验一层，继续输入：`丞相状态`'
...
try {
    $driftedChecklistContent = (Get-Content $codexHomeExportPanelChecklistPath -Raw).Replace($checklistStepSummaryLineText, '5. 最后输入：`丞相状态`')
    [System.IO.File]::WriteAllText($codexHomeExportPanelChecklistPath, $driftedChecklistContent, $utf8NoBom)
