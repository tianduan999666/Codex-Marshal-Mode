...
2. 面板内默认先走 `start-panel-task.ps1`：先做轻量检查；必要时再转入完整验真与自动修复；通过后写入本机验真状态。
...
- `传令：` 是默认自然语言任务入口；`传令 帮助 / 传令 状态 / 传令 检查 / 传令 修复 / 传令 验板 / 传令 版本` 是固定面板命令。
- 对外主称统一为 `丞相`；状态栏中的 `当前模式` 固定显示 `丞相` 或 `都督`，用于区分执行态与维护态。
- 一句话入口固定链路：`轻量检查 → 完整验真（必要时） → 自动修复（必要时） → 自动建任务 → 进入执行模式`。
- `验真`、`修复`、`检查` 只针对丞相运行环境与真源，不检查你的项目代码。
